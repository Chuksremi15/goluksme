    ...
    <div className="flex text-sm font-medium text-center mx-4 gap-x-4 items-center">
      {!isCard && (
        <button
          ...
          className="bg-primary text-xs font-semibold rounded-full px-3 py-1"
        >
          Max
        </button>
      )}
      ...
      <div className={`  flex items-center relative`}>
        {stableCoinSymbol && (
          <h4 className="text-sm font-normal text-gray">{stableCoinSymbol}</h4>
        )}
        ...
        <input
          ...
          id="amount"
          aria-label="amount"
          className={` ${inputAmount ? "" : "text-gray2"} text-lg bg-transparent text-center max-w-[150px] p-0 sm:leading-6 focus:outline-none focus:border-none focus:ring-0 `}
          inputMode="decimal"
          value={inputAmount}
